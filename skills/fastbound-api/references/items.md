...
| Field | Notes |
|---|---|
| `externalId` | Your per-firearm ID. Prefer storing FastBound's Item ID in your own system over relying on this; use `externalId` only when that isn't possible. **Must NOT be GUID-shaped** (32-char hex or hyphenated GUID form) — those collide with FastBound's GUID routing. Use a prefix, suffix, or non-hex encoding. See `common-patterns.md`. |
...
This is for integrations that must use `externalId` — a legacy system whose schema can't be extended to hold FastBound's Item IDs. If you can store FastBound's IDs instead, do that and skip this entirely; see `common-patterns.md`.
