...
1. **Have the contact ready.** Retrieve it by the FastBound Contact ID you stored, look it up by `externalId`, or create new. See `contacts.md`. For non-licensee sales captured via 4473, the contact is typically created by FastBound's UI when the buyer fills out the form.
...
| Field | Type | Notes |
|---|---|---|
| `externalId` | string | Your system's ID. Secondary index — store FastBound's Disposition ID as the primary link. See `common-patterns.md`. |
